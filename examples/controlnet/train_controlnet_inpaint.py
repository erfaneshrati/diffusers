...
#        masks = []
#        masked_images = []
#        for image, mask in zip(examples[image_column], examples[conditioning_image_column]):
...
def collate_fn(examples):
    ...
    for i in range(len(pixel_values)):
        ...
        ratio = 768.0 / image.shape[0:2][dim_min_ind]
        dim[1-dim_min_ind] = 768
        ...
        pixel_values[i] = Image.fromarray(image)
...
#        pixel_values[i].save('pixel_values.jpeg')
#        conditioning_images[i].save('conditioning_image.jpeg')
        #masked_image.save('masked_image.jpeg')
        #mask.save('mask.jpeg')
#    conditioning_images[i].save('conditioning_image_orig.jpeg')
    #for i in range(len(conditioning_images)):
    #    conditioning_images[i].paste(im=pixel_values[i], box=(0,0), mask=ImageOps.invert(conditioning_images[i]).convert('L'))
#    conditioning_images[i].save('conditioning_image.jpeg')
#    pixel_values[i].save('pixel_values.jpeg')
...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = ControlNetModel.from_unet(UNet2DConditionModel.from_pretrained("runwayml/stable-diffusion-inpainting", subfolder="unet", revision=args.revision))
